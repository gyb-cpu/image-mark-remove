  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials): Promise<User | null> {
        ...
        const { email, password, isRegister } = credentials as { email: string; password: string; isRegister: string }
        ...
        if (kv) {
          if (isRegister === "true") {
            ...
            return { id, email: email as string }
          } else {
        ...
        if (isRegister === "true") {
          ...
          return { id: newUser.id, email: newUser.email as string }
        } else {
          if (!existingUser || existingUser.password !== password) throw new Error("Invalid credentials")
          return { id: existingUser.id, email: existingUser.email as string }
        }
      },
    }),
  ],
